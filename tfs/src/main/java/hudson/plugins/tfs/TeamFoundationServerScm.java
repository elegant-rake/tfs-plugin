...
public class TeamFoundationServerScm extends SCM {
    ...
    public Collection<String> getCloakedPathsCollection() {
        return this.cloakedPaths != null ? this.cloakedPaths : new ArrayList<String>();
    }
    ...
    public void checkout(@Nonnull Run<?, ?> build, @Nonnull Launcher launcher, @Nonnull FilePath workspace, @Nonnull TaskListener listener, @CheckForNull File changelogFile, @CheckForNull SCMRevisionState baseline) throws IOException, InterruptedException {
        ...
        try {
            WorkspaceConfiguration workspaceConfiguration = new WorkspaceConfiguration(server.getUrl(), getWorkspaceName(), getProjectPath(), getCloakedPathsCollection(), getLocalPath());

            Run<?,?> previousBuild = build.getPreviousBuild();

            if (previousBuild != null) {
                BuildWorkspaceConfiguration nodeConfiguration = new BuildWorkspaceConfiguration(workspaceConfiguration, build);

                if (nodeConfiguration.workspaceExists() && !workspaceConfiguration.equals(nodeConfiguration)) {
            ...
            final int changeSet = recordWorkspaceChangesetVersion(build, listener, project, projectPath);

            CheckoutAction action = new CheckoutAction(workspaceConfiguration, isUseUpdate(), isUseOverwrite());
            List<ChangeSet> list;
            ...
            list = action.checkout(server, workspace, previousBuildVersionSpec, currentBuildVersionSpec);
    ...
    int recordWorkspaceChangesetVersion(final Run<?, ?> build, final TaskListener listener, final Project project, final String projectPath) throws IOException, InterruptedException {
